...
url = "https://www.trademe.co.nz/a/property/residential/sale/wellington/wellington/raroa"
driver.get(url)
time.sleep(5)
...
for navlinks in navigationlinks:
    navurl = "https://www.trademe.co.nz/a/property/residential/sale/wellington/wellington/raroa" + navlinks
    driver.get(navurl)
    time.sleep(5)
    html_content = driver.page_source
    soup = BeautifulSoup(html_content, "html.parser")
...
for link in pagelinks:
    ...
    cursor = db.cursor()
    query = "SELECT * FROM propertylist_raroa WHERE addr = %s AND suburb = %s AND region = %s AND city = %s"
    ...
    if len(propertyrecord) == 0:
        ...
        cursor = db.cursor()
        query = "INSERT INTO propertylist_raroa SET addr = %s, suburb = %s, region = %s, city = %s, price = %s, active = 1"
        ...
        cursor = db.cursor()
        query = "INSERT INTO active_listings_raroa SET propid = %s, price = %s, link = %s, lastscan = %s"
    ...
    # If the property is known
        ...
        if propertyrecord[0][6] == 0:
            ...
            cursor = db.cursor()
            query = "UPDATE propertylist_raroa SET active = 1 WHERE id = %s"
        ...
        cursor = db.cursor()
        query = "SELECT * FROM active_listings_raroa WHERE propid = %s"
        val = (propertyrecord[0][0],)
        cursor.execute(query, val)
        ...
        if len(activerecord) > 0:
            ...
            if activerecord[0][2] == price and activerecord[0][3] == link:
                ...
                # Only update the last scan time
                cursor = db.cursor()
                query = "UPDATE active_listings_raroa SET lastscan = %s WHERE id = %s"
            ...
            # Re-listed properties
                ...
                cursor = db.cursor()
                query = "INSERT INTO prior_listings_raroa SET propid = %s, price = %s, link = %s, timeadded = %s"
                val = (activerecord[0][1], price, link, currenttime)
                cursor.execute(query, val)
                db.commit()
                cursor.close()
                ...
                cursor = db.cursor()
                query = "DELETE FROM active_listings_raroa WHERE id = %s"
